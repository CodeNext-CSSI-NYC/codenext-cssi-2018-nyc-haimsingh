...
// Hint: Look up how to use str.includes(otherStr), a string method that checks
// if a string
// contains another string(which could be one or more letters).
// 	(You shouldn 't use an object to do this, it'
// 		s a warm up.)
//
// let empty = []
// for (var i = 0; i < words.length; i++) {
// 	// console.log(words);
// 	if (words[i].includes("b") && words[i].includes("s")) {
// 		empty.push(words[i]);
// 	}
// }
// console.log(empty);
...
// 3. Similar to what we did before, create a
...
// console.log(makeVal("mississippi"));
...
function ifAna(str) {
	let stringTxt = makeVal(str);
	// console.log(stringTxt);
	for (var i = 0; i < words.length; i++) {
		let wordsTxt = makeVal(words[i]);

		if (objectEqual(wordsTxt, stringTxt)) {
			// console.log(str);
			console.log(words[i]);
		}
	}
